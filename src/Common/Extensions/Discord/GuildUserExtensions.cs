...
namespace CoupBot.Common.Extensions.Discord
{
    public static class GuildUserExtensions
    {
        public static async Task<int> GetMessageCount(this IGuildUser guildUser)
        {
            ...
            foreach (var textChannel in
                await guildUser.Guild.GetTextChannelsAsync()) // for every text channel in the guild
            {
                var totalMessages =
                    await textChannel.GetMessagesAsync(int.MaxValue).FlattenAsync(); // get all messages in that channel
                var messageCountFromUser =
                    totalMessages.Count(x => x.Author.Id == guildUser.Id); // get number of messages from user
